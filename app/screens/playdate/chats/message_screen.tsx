...
const MessageScreen = () => {
  ...
  const inputPaddingBottom = useRef(
    new Animated.Value(dimensions.screenHeight * 0.05)
  ).current;
  ...
  const renderItem = ({ item, index }: { item: any; index: number }) => {
    ...
    const isLastOfBlock =
      !nextItem || nextItem.sender_pet_id !== item.sender_pet_id;
    const isStartOfBlock =
      !prevItem || prevItem.sender_pet_id !== item.sender_pet_id;
    ...
    const shouldShowSent =
      isMine && !isPending && latestRealMessage?.id === item.id;
    ...
    return (
      <View
        style={{
          flexDirection: "row",
          alignItems: "flex-end",
          marginBottom: isLastOfBlock ? 10 : 2,
        }}
      >
        {!isMine && !isLastOfBlock && (
          <View style={{ width: dimensions.screenWidth * 0.12 }} />
        )}
        {!isMine && isLastOfBlock && (
          <View style={styles.avatarContainer}>
            <Image source={{ uri: otherPetAvatar }} style={styles.avatar} />
            ...
            <View style={{ marginRight: dimensions.screenWidth * 0.01 }}>
              {!isPending && (
                <Text style={styles.sendingText}>
                  Seen at {moment(item.read_at, "HH:mm").format("h:mm A")}
                </Text>
              )}
            </View>
          )}
